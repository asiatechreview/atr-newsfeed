...
export async function lookupItem(env, itemParam) {
  ...
  try {
    const row = await env.ATR_FEED_DB.prepare(
      "SELECT id, headline, blurb, source_name, source_url, link_key, published_at, category FROM feed_items WHERE status = 'published' AND (id = ?1 OR LOWER(link_key) = LOWER(?2)) LIMIT 1"
    )
